...
Migrations run **automatically** on container start — `backend/entrypoint.sh`
waits for Postgres, runs `alembic upgrade head`, then starts uvicorn. No manual
step needed on first run.

To create the first admin (required once, since staff creation is admin-only):

```bash
docker compose exec backend python -m scripts.seed_admin \
  --name "Your Name" --email you@yourdomain.com --password "changeme123"
```

### Running migrations manually (e.g. after pulling a new migration)
